...
class FasterRCNNFeatureExtractor(object):
  ...
  def restore_from_classification_checkpoint_fn(
    ...
    print(first_stage_feature_extractor_scope)
...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def predict(self, preprocessed_inputs, true_image_shapes):
    ...
    """
    preprocessed_inputs = tf.Print(preprocessed_inputs,[tf.shape(preprocessed_inputs)])
    (rpn_box_predictor_features, rpn_features_to_crop, anchors_boxlist,
     image_shape) = self._extract_rpn_feature_maps(preprocessed_inputs)
    rpn_features_to_crop = tf.Print(rpn_features_to_crop,[tf.shape(rpn_features_to_crop)])
    (rpn_box_encodings, rpn_objectness_predictions_with_background
    ) = self._predict_rpn_proposals(rpn_box_predictor_features)
    
    tensor_shape = tf.shape(rpn_features_to_crop)
    with tf.variable_scope('add_atten'):
        with slim.arg_scope([slim.conv2d], 
                  weights_initializer=tf.truncated_normal_initializer(0.0, 0.01),
                  weights_regularizer=slim.l2_regularizer(0.005), padding='SAME'):
            net = slim.conv2d(rpn_features_to_crop, 10, [1, 1],activation_fn=tf.nn.tanh, scope='convX')
            net = slim.conv2d(net, 1, [1, 1],activation_fn=tf.nn.tanh, scope='conv1')
            atten = tf.nn.relu(net)  
        
    if self._is_training:
        atten2 = tf.reshape(atten, (-1,tensor_shape[1]*tensor_shape[2]))
        area2 = tf.image.resize_images(self.area, (tensor_shape[1], tensor_shape[2]))
        area2 = tf.unstack(area2, axis=3)[0]  
        area = tf.reshape(area2, (-1,tensor_shape[1]*tensor_shape[2]))
        cost = tf.reduce_mean(tf.pow(area - atten2, 2)) 
        cost = tf.Print(cost,[cost, tf.reduce_mean(area),'atten_cost'])
#     else:
    else:
        atten3 = tf.tile(atten,[1,1,1,12])
        atten3 = tf.reshape(atten3,(tensor_shape[1]*tensor_shape[2]*12,))
        wh_atten_index = tf.reshape(tf.where(tf.greater(atten3,0)),[-1])
        anchors_boxlist.data['boxes'] = tf.gather(anchors_boxlist.data['boxes'], wh_atten_index)
        rpn_box_encodings = tf.squeeze(rpn_box_encodings, axis=0)
        rpn_box_encodings = tf.gather(rpn_box_encodings, wh_atten_index)
        rpn_box_encodings = tf.expand_dims(rpn_box_encodings, axis=0)
...
#     rpn_features_to_crop = tf.Print(rpn_features_to_crop,[tf.shape(rpn_features_to_crop),'1'])
    ...
    if self._is_training:
        prediction_dict['atten_loss'] = cost
  ...
  def _extract_rpn_feature_maps(self, preprocessed_inputs):
    ...
    anchors = box_list_ops.concatenate(
        self._first_stage_anchor_generator.generate([(feature_map_shape[1],
                                                      feature_map_shape[2])]))
    a  = self._first_stage_anchor_generator.generate([(feature_map_shape[1],
                                                      feature_map_shape[2])])
#     rpn_features_to_crop = tf.Print(rpn_features_to_crop, [tf.shape(rpn_features_to_crop)[1:],len(a),a[0].get()[0], tf.shape(a[0].get()),tf.shape(anchors.get())[:2],tf.shape(anchors.get())[2:],'------anchors'])
  ...
  def loss(self, prediction_dict, area, true_image_shapes, scope=None):
    ...
    """
#     with tf.variable_scope('add_atten'):
#         area = tf.unstack(area, axis=3)[0]        
#         tensor_shape = tf.shape(prediction_dict['rpn_features_to_crop'])
#         atten = slim.conv2d(prediction_dict['rpn_features_to_crop'],1, [1,1], stride = [1,1],padding='SAME', activation_fn =tf.nn.tanh,scope='attention')
#         atten = tf.reshape(atten, (-1,tensor_shape[1]*tensor_shape[2]))
#         area2 = tf.image.resize_images(area, (tensor_shape[1], tensor_shape[2]))
#         area = tf.reshape(area2, (-1,tensor_shape[1]*tensor_shape[2]))
#         cost = tf.reduce_mean(tf.pow(area - atten, 2)) 
#         cost = tf.Print(cost,[cost, tf.reduce_mean(area),'atten_cost'])
#     tf.losses.add_loss(cost)
    
#     true_image_shapes = tf.Print(true_image_shapes, [true_image_shapes,'3'])
...
#       loss_dict['atten'] = cost;
  ...
  def restore_map(self,
    ...
    return {var.op.name: var for var in feature_extractor_variables if not ('SecondStageFeatureExtractor/InceptionV2/Mixed_5a' in var.op.name or 'add_atten' in var.op.name) }

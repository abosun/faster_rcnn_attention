...
def dict_to_tf_example(data,
    ...
    """
    img_path = os.path.join(dataset_directory, image_directory, data['filename'].split('.')[0]+'.jpg')
    ...
    poses = []
    difficult_obj = []
    mask = np.zeros((width, height),np.uint8)+128
    for obj in data['object']:
        ...
        mask[int(obj['bndbox']['xmin']):int(obj['bndbox']['xmax']),int(obj['bndbox']['ymin']):int(obj['bndbox']['ymax'])]=255
    ...
    area = PIL.Image.fromarray(mask)
    area.save('tmp.jpg')
    with tf.gfile.GFile('tmp.jpg', 'rb') as fid:
        area_jpg = fid.read()
    example = tf.train.Example(features=tf.train.Features(feature={
        'image/maskarea':dataset_util.bytes_feature(area_jpg),

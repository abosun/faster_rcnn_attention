...
#     image = tf.Print(image,["<<", read_data[fields.InputDataFields.filename]])
    ...
    print(read_data)
    print(masks_gt,'000<<<<')
    return (image,area, key, location_gt, classes_gt, masks_gt, keypoints_gt,
            weights_gt)
...
def _create_losses(input_queue, create_model_fn, train_config):
  ...
  (images, area, _, groundtruth_boxes_list, groundtruth_classes_list,
...
#     true_image_shapes.append(true_image_shape)
  image = images[0]
  sp = tf.shape(image)
  area = tf.to_float(area) 
  area = tf.image.resize_images(area, (sp[1],sp[2]))
  image2 = tf.concat([image, area],axis=3)
  resized_image0, true_image_shape = detection_model.preprocess(image2)
  [resized_image,area] = tf.split(resized_image0, 2, 3)
  preprocessed_images.append(resized_image)
  true_image_shapes.append(true_image_shape)

#   area = tf.Print(area, [tf.shape(area),'03'])
  ...
  detection_model.area = area
  prediction_dict = detection_model.predict(images, true_image_shapes)

  losses_dict = detection_model.loss(prediction_dict, area, true_image_shapes)
  for loss_tensor in losses_dict.values():
    tf.losses.add_loss(loss_tensor)
  tf.losses.add_loss(prediction_dict['atten_loss'])
...
def train(create_tensor_dict_fn,
  ...
  with tf.Graph().as_default():
    ...
    print("!!!!!", train_config.fine_tune_checkpoint_type)
    slim.learning.train(
        train_tensor,
        logdir=train_dir,
        master=master,

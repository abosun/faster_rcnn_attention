...
PATH_TO_LABELS = os.path.join('./object_detection/train_checkpoint_prefix_3', 'disk_label_map.pbtxt')
NUM_CLASSES = 3
...
if (len(sys.argv)>1):
  ...
  test_dir = 'test-public'
lines0 = [x.split(',')[0] for x in open('./object_detection/sub_highest.csv').readlines()]
lines = list(set(lines0))
lines.sort(key=lines0.index)
TEST_IMAGE_PATHS = [os.path.join(test_dir,x) for x in lines]
obxes = []
obxes1 = []
with detection_graph.as_default():
